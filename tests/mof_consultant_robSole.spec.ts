...
//Account Information
    ...
    await page.waitForLoadState('networkidle'); //wait for page load
    await page.locator('[id="_onlineRegistration_WAR_NGePportlet_:form:suppTypeId_panel"]').getByText('Consultant').click();
    await page.waitForTimeout(3000); // Wait 3 seconds
...
// Professional Body Information
    ...
    // 1. Click the label containing 'profBodyName_label'
    await page.locator('[id*="profBodyName_label"]').click();
        await page.waitForTimeout(1500); // Wait 1.5 seconds
    // 2. Select the option from the panel containing 'profBodyName_panel'
    await page.locator('[id*="profBodyName_panel"]')
        .getByText('KELULUSAN JURUAUDIT')
        .click();
        await page.waitForTimeout(1500); // Wait 1.5 seconds
    
    const inputLocator = page.locator('[id*="assocBodyRegNoId"]');
        await inputLocator.click();
        await inputLocator.fill('0123456789');

    //Date Picker - Expiry Date
    await page.locator('[id*="assocBodyExpDateId"]')
        .getByRole('button', { name: 'ui-button' })
        .click();
